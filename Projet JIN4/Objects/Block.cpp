...
const char Block::collision(sf::FloatRect& boundingBox) const {
    ...
    if (bounds.intersects(boundingBox)) {
        // if the top or bottom middle point of the ball is in the block rectangle, it's a vertical collision
        if (bounds.contains(center.x, boundingBox.top) || bounds.contains(center.x, boundingBox.top + boundingBox.height)) { return 'v'; }
        // if the left or right middle point of the ball is in the block rectangle, it's a horizontal collision
        else if (bounds.contains(boundingBox.left, center.y) || bounds.contains(boundingBox.left + boundingBox.width, center.y)) { return 'h'; }
        ...
        //otherwise, the boxes intersect but there is no actual collision, nothing 2B done
    }
    return 'n';
}
